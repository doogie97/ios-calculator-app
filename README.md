...
그런데 UML에 끼워 맞추다 보니 불필요한 과정이 너무 많이 생기기 시작
예를 들면
```swift
...
(애초에 Formula로 반환하는 것이 parse() 함수에서 직접 넣어 줄 수 있으면 위 방법도 좋았겠으나 기존 UML에 필요한 타입등은 추가가 가능하나 이미 설정된 내용은 수정이 불가하기에 전체적으로 뒤엎고 현재와 같이 연산자, 피연산자를 하나의 스트링으로 받아 = 을 눌렀을 때 한번만 반환 하도록 구현)
